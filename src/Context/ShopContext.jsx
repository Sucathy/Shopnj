...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    fetch("http://54.204.66.236:4000/allproducts")
      .then((res) => res.json())
      .then((data) => setProducts(data));

    if (localStorage.getItem("auth-token")) {
      fetch("http://54.204.66.236:4000/getcart", {
        method: "POST",
        headers: {
  ...
  const addToCart = (itemId) => {
    ...
    if (localStorage.getItem("auth-token")) {
      fetch("http://54.204.66.236:4000/addtocart", {
        method: "POST",
        headers: {
          Accept: "application/form-data",
          "auth-token": `${localStorage.getItem("auth-token")}`,
  ...
  const removeFromCart = (itemId) => {
    setCartItems((prev) => ({ ...prev, [itemId]: prev[itemId] - 1 }));
    if (localStorage.getItem("auth-token")) {
      fetch("http://54.204.66.236:4000/removefromcart", {
        method: "POST",
        headers: {
          Accept: "application/form-data",
          "auth-token": `${localStorage.getItem("auth-token")}`,
...
=======
      fetch("http://54.204.66.236:4000/clearcart", {
>>>>>>> origin/main
        method: "POST",
        headers: {
          Accept: "application/form-data",
