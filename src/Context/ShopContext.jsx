...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("auth-token")) {
      fetch("http://34.229.212.238:4000/getcart", {
        method: "POST",
        headers: {
  ...
  const addToCart = (itemId) => {
    ...
    if (localStorage.getItem("auth-token")) {
      fetch("http://34.229.212.238:4000/addtocart", {
        method: "POST",
        headers: {
          Accept: "application/form-data",
          "auth-token": `${localStorage.getItem("auth-token")}`,
  ...
  const removeFromCart = (itemId) => {
    setCartItems((prev) => ({ ...prev, [itemId]: prev[itemId] - 1 }));
    if (localStorage.getItem("auth-token")) {
      fetch("http://34.229.212.238:4000/removefromcart", {
        method: "POST",
        headers: {
          Accept: "application/form-data",
          "auth-token": `${localStorage.getItem("auth-token")}`,
  ...
  const clearCart = () => {
    ...
    if (localStorage.getItem("auth-token")) {
      fetch("http://34.229.212.238:4000/clearcart", {
        method: "POST",
        headers: {
          Accept: "application/form-data",
          "auth-token": `${localStorage.getItem("auth-token")}`,
