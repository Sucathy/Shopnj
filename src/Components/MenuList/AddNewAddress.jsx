...
const AddNewAddress = () => {
  ...
  const fetchAccountDetails = async () => {
    try {
      const response = await fetch("http://44.202.54.225:4000/addressdetails", {
        method: "GET",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://44.202.54.225:4000/address", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("auth-token") || "",
  ...
  const handleDelete = async (addressId) => {
    try {
      const response = await fetch(
        `http://44.202.54.225:4000/address/${addressId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
